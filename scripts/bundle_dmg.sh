...
#
# Vendored from tauri-bundler (it drops this into target/ when it builds a dmg). Kept in the repo
# because release.sh builds the dmg itself: the app has to be notarised and stapled *before* the
# dmg is made, and `tauri build --bundles dmg` rebuilds and re-signs the app, discarding the
# ticket. A `--bundles app` build never writes this file, so it cannot be relied on from target/.
#
# Unmodified apart from this comment. Re-copy it from target/release/bundle/dmg/ after a Tauri
# upgrade if its dmg layout changes.
